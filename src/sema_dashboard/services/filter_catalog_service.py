...
def _prepare_inventario_catalog() -> pd.DataFrame:
    ...
    if inventario.empty:
        return pd.DataFrame(columns=["externo", "zona_auto"])
    ...
    return inventario[["externo", "zona_auto"]].drop_duplicates().copy()
...
def _prepare_estados_catalog() -> pd.DataFrame:
    ...
    if estados.empty:
        return pd.DataFrame(
            columns=["externo", "direccion", "zona_auto", "estado_concert", "gestion_sema"]
        )
    ...
    return estados[["externo", "direccion", "zona_auto", "estado_concert", "gestion_sema"]].drop_duplicates().copy()
...
def _build_catalog_base(fecha_inicio=None, fecha_fin=None) -> pd.DataFrame:
    ...
    if metadata.empty:
        metadata = pd.DataFrame(columns=["externo", "direccion", "zona_auto", "estado_concert", "gestion_sema"])
    ...
    for column in ["externo", "direccion", "zona_auto", "estado_concert", "gestion_sema", "acceso"]:
...
def get_filter_options(filters: dict) -> dict[str, list[str]]:
    ...
    for key in ["externo", "direccion", "acceso", "zona_auto", "estado_concert", "gestion_sema"]:
